	...
	for i in range(0, size):
		if rank == i:
			...
			for epoch in range(nb_epochs):
				...
				# each worker extracts entire weights of his model
				weights = train_model.get_weights()

				# each worker iterates through his weights tensor and selects weights that are 
				# above given threshold 
				selected_weights = []
				for weight in weights:
					bool_mask = (weight > thres_hold).astype(int)
					selected_weights.append(weight*bool_mask)

				
				# workers send selected weights to each other
				for k in range(0, size):
					if rank == k:
						for neighbor in neighbors:
							comm.send(selected_weights, dest=neighbor)
					else:
						w = []
						w.append(weights)
						recv_selected_weights=comm.recv(source=k)
						for index in range(len(recv_selected_weights)):
							if (recv_selected_weights[index]).all() == -0.:
								recv_selected_weights[index] = weights[index]
						w.append(recv_selected_weights)
						# find the average of recieved weights
