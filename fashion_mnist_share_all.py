	...
	for i in range(0, size):
		if rank == i:
			...
			for epoch in range(nb_epochs):
				...
				# workers send thier weights to each other
				for k in range(0, size):
					if rank == k:
						for neighbor in neighbors:
							comm.send(weights, dest=neighbor)
					else:
						w = []
						w.append(weights)
						recv_weights=comm.recv(source=k)
						w.append(recv_weights)
						# find the average of recieved weights
						model_weights = average_weights(w)
						# set the weights to the model
						model.set_weights(model_weights)
						
				np.savetxt("model_loss"+str(i)+".csv", model_loss, delimiter=",")
				np.savetxt("model_accuracy"+str(i)+".csv", model_accuracy, delimiter=",")
				np.savetxt("validation_loss"+str(i)+".csv", validation_loss, delimiter=",")
				np.savetxt("validation_accuracy"+str(i)+".csv", validation_accuracy, 	delimiter=",")
